 ...
 * </p>
 * <ul>
 * <li>v2.3.7 - Fixed subtle bug when base 64 input stream contained the value 01111111, which is an invalid base 64 character but should not throw an
 * ArrayIndexOutOfBoundsException either. Led to discovery of mishandling (or potential for better handling) of other bad input characters. You should now get
 * an IOException if you try decoding something that has bad characters in it.</li>
 * <li>v2.3.6 - Fixed bug when breaking lines and the final byte of the encoded string ended in the last column; the buffer was not properly shrunk and
 ...
 * <li>v2.3.4 - Fixed bug when working with gzipped streams whereby flushing the Base64.OutputStream closed the Base64 encoding (by padding with equals signs)
 * too soon. Also added an option to suppress the automatic decoding of gzipped streams. Also added experimental support for specifying a class loader when
 * using the {@link #decodeToObject(java.lang.String, int, java.lang.ClassLoader)} method.</li>
 * <li>v2.3.3 - Changed default char encoding to US-ASCII which reduces the internal Java footprint with its CharEncoders and so forth. Fixed some javadocs that
 * were inconsistent. Removed imports and specified things like java.io.IOException explicitly inline.</li>
 * <li>v2.3.2 - Reduced memory footprint! Finally refined the "guessing" of how big the final encoded data will be so that the code doesn't have to create two
 * output arrays: an oversized initial one and then a final, exact-sized one. Big win when using the {@link #encodeBytesToBytes(byte[])} family of methods (and
 * not using the gzip options which uses a different mechanism with streams and stuff).</li>
 * <li>v2.3.1 - Added {@link #encodeBytesToBytes(byte[], int, int, int)} and some similar helper methods to be more efficient with memory by not returning a
 * String but just a byte array.</li>
 * <li>v2.3 - <strong>This is not a drop-in replacement!</strong> This is two years of comments and bug fixes queued up and finally executed. Thanks to everyone
 * who sent me stuff, and I'm sorry I wasn't able to distribute your fixes to everyone else. Much bad coding was cleaned up including throwing exceptions where
 * necessary instead of returning null values or something similar. Here are some changes that may affect you:
 * <ul>
 ...
 * <li><em>Throws exceptions instead of returning null values.</em> Because some operations (especially those that may permit the GZIP option) use IO streams,
 * there is a possiblity of an java.io.IOException being thrown. After some discussion and thought, I've changed the behavior of the methods to throw
 * java.io.IOExceptions rather than return null if ever there's an error. I think this is more appropriate, though it will require some changes to your code.
 ...
 * <li>Cleaned up as much javadoc as I could to avoid any javadoc warnings. This was especially annoying before for people who were thorough in their own
...
public class Base64 {
	...
	/**
	 * Returns one of the _SOMETHING_ALPHABET byte arrays depending on the options specified. It's possible, though silly, to specify ORDERED <b>and</b> URLSAFE
	 * in which case one of them will be picked, though there is no guarantee as to which one will be picked.
	 */
	 ...
	 * bytes in your array is given by <var>numSigBytes</var>. The array <var>threeBytes</var> needs only be as big as <var>numSigBytes</var>. Code can reuse a
	 * byte array by passing a four-byte array as <var>b4</var>.
	 *
	...
	/**
	 * <p>
	 * Encodes up to three bytes of the array <var>source</var> and writes the resulting four Base64 bytes to <var>destination</var>. The source and destination
	 * arrays can be manipulated anywhere along their length by specifying <var>srcOffset</var> and <var>destOffset</var>. This method does not check to make
	 * sure your arrays are large enough to accomodate <var>srcOffset</var> + 3 for the <var>source</var> array or <var>destOffset</var> + 4 for the
	...
	/**
	 * Performs Base64 encoding on the <code>raw</code> ByteBuffer, writing it to the <code>encoded</code> ByteBuffer. This is an experimental feature.
	 * Currently it does not pass along any options (such as {@link #DO_BREAK_LINES} or {@link #GZIP}.
	 *
	...
	/**
	 * Performs Base64 encoding on the <code>raw</code> ByteBuffer, writing it to the <code>encoded</code> CharBuffer. This is an experimental feature.
	 * Currently it does not pass along any options (such as {@link #DO_BREAK_LINES} or {@link #GZIP}.
	 *
	 * @param raw input buffer
	 * @param encoded output buffer
	 ...
	 *
	 * <p>
	 * As of v 2.3, if the object cannot be serialized or there is another error, the method will throw an java.io.IOException. <b>This is new to v2.3!</b> In
	 ...
	 *
	 * <p>
	 * As of v 2.3, if there is an error with the GZIP stream, the method will throw an java.io.IOException. <b>This is new to v2.3!</b> In earlier versions, it
	 * just returned a null value, but in retrospect that's a pretty poor way to handle it.
	 * </p>
	 *
	 *
	 * @param source The data to convert
	 ...
	 *
	 * <p>
	 * As of v 2.3, if there is an error, the method will throw an java.io.IOException. <b>This is new to v2.3!</b> In earlier versions, it just returned a null
	...
	/**
	 * Similar to {@link #encodeBytes(byte[], int, int, int)} but returns a byte array instead of instantiating a String. This is more efficient if you're
	 ...
	 * check to make sure your arrays are large enough to accomodate <var>srcOffset</var> + 4 for the <var>source</var> array or <var>destOffset</var> + 3 for
	 * the <var>destination</var> array. This method returns the actual number of bytes that were converted from the Base64 encoding.
	 * <p>
	...
	/**
	 * Low-level access to decoding ASCII characters in the form of a byte array. <strong>Ignores GUNZIP option, if it's set.</strong> This is not generally a
	 * recommended method, although it is used internally as part of the decoding process. Special case: if len = 0, an empty array is returned. Still, if you
	 * need more speed and reduced memory footprint (and aren't gzipping), consider this method.
	 *
	...
	/**
	 * Attempts to decode Base64 data and deserialize a Java Object within. Returns <tt>null</tt> if there was an error. If <tt>loader</tt> is not null, it will
	 ...
	 *
	 * <p>
	 * As of v 2.3, if there is a error, the method will throw an java.io.IOException. <b>This is new to v2.3!</b> In earlier versions, it just returned false,
	...
	public static String encodeFromFile(File file) throws java.io.IOException {
